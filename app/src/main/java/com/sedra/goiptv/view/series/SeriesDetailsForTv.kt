...
//        val seasons =  seriesDetails.seasons!!.filter { !it.name?.contains("Specials",true)!! }
        ...
        binding.addSeriesToFav.setOnClickListener {
            if (isFavourite) {
                isFavourite = false
                binding.addSeriesToFav.text = getString(R.string.add_to_favourite)
                removeFavourite(seriesDetails)
            } else {
                isFavourite = true
                binding.addSeriesToFav.text = getString(R.string.remove_from_fav)
        ...
        favouritesViewModel.allFavourites(this).observe(this) {
            if (it != null) {
                if (it.any { item -> item.id == intExtra }) {
                    binding!!.addSeriesToFav.text = "حذف من المفضل"
                    isFavourite = true
                }
            }
        }
    ...
    private fun removeFavourite(series: SeriesDetailsResponse?) {
        ...
        favouritesViewModel.removeFavourite(this, item)
    }
